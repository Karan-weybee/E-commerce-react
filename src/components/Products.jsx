...
const Products = () => {
  ...
  async function getProducts() {
    const querySnapshot = await getLimitedProducts(dataLimit);
  ...
  async function getCategories(docs, card) {
    await getProductCategory(docs).then(async (document) => {
      if (
        document.exists() &&
        document.data() &&
        Array.isArray(document.data().categories)
      ) {
        let categoryFormDb = [];

        await Promise.all(
          document.data().categories.map(async (category) => {
            const docu = await getCategory(category);
        ...
        if (valid) {
          if (
            priceRange[0] <=
              docs.data().price -
                (docs.data().price * docs.data().discount) / 100 &&
            docs.data().price -
              (docs.data().price * docs.data().discount) / 100 <=
              priceRange[1]
          ) {
  ...
  function handleCheckboxChange(category) {
    if (document.getElementById(category).checked) {
      ...
      setCheckedList((checkedCategories) =>
  ...
  function openFilter() {
    document.querySelector(".filter-section").style.animationName =
      "filterSection";
      ...
      <section id="products" className="container section-products">
        ...
        <div className="filterBtnSection">
          <button className="filterBtn" onClick={openFilter}>
            <img src={sortIcon} alt="" className="filter-icon" />
            <span> SORT</span>
          </button>
            ...
            {filterCategory.map((categorys) => (
              <span key={categorys}>
                ...
                <button
                  ...
                  onClick={(e) => clearFilter(e.target.id)}
                >
